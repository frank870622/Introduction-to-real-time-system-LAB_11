...
int start_schedule(int); //schedule part

void time_demand(vector<Task> waitvector, int);
...
bool sort_period_time(Task, Task);               //sort vector's period from small to big
...
int main()
{
    ...
    int schedule_num = 1;
    int start_schedule_return; //-1: eof, 0:ok, 1: can't schedule
    while (1)
    {
        start_schedule_return = start_schedule(schedule_num);
        if (start_schedule_return == -1)
            break;
        else if (start_schedule_return == 0)
        {
            //cout << "Task case " << schedule_num << ": schedulable" << endl;
        }
        else if (start_schedule_return == 1)
        {
            //cout << "Task case " << schedule_num << ": not schedulable" << endl;
        }
        process_time = 0;
        ++schedule_num;
    }
...
int start_schedule(int schedule_num)
{
    ...
    float base_utilization = 0;        //U(task num) for lab 10
    float task_utilization = 0;        //Ui for task (lab 10);
    ...
    //input processor number & task number
    if (cin >> processor_num >> task_num)
        ;
    else
        return -1;

    base_utilization = (float)task_num * (pow(2, 1 / (float)task_num) - 1);
    cout << "base_utilization: " << base_utilization << endl;
    ...
    for (int i = 0; i < processor_num; i++)
    {
        ...
        cin >> processor.id; //input each processor's ID & ability
        cin >> processor.ability;
    ...
    for (int i = 0; i < task_num; i++)
    {
        ...
        //lab 10 get utilization
        task_utilization += (float)task.execution_time / (float)task.period;

        waitvector.push_back(task); //push task into vector
    }

    time_demand(waitvector, schedule_num);

    //cout << "after task input" << endl;
    cout << "task_utilization: " << task_utilization << endl;
    ...
    //if task_utilization > base_utilization -> not schedulable
    if (task_utilization > base_utilization)
        return 1;
    ...
    while (1)
    {
        ...
        sort(readyvector_low.begin(), readyvector_low.end(), sort_period_time);
        sort(waitvector.begin(), waitvector.end(), sort_realease_time);
        ...
        if (waitvector.size() > 0)
        {
            ...
            while (waitvector.begin()->release_time <= process_time && waitvector.size() > 0)
            {
                ...
                sort(readyvector_low.begin(), readyvector_low.end(), sort_period_time); //sort the ready queue by deadline
        ...
        for (int i = 0; i < processor_num; i++)
        {
            ...
            if (runvector[i].size() <= 0)
            {
                ...
                else if (readyvector_low.size() > 0)
                {
                    ...
                    sort(readyvector_low.begin(), readyvector_low.end(), sort_period_time); //sort ready queue with deadline
                ...
                else if (readyvector_low.size() > 0 && 0) //lab9 : frame
                {                                         //lab9 mode
                    //this scheme let ready queue sort first by deadline, second by id
                    sort(readyvector_low.begin(), readyvector_low.end(), sort_ID);          //sort id to let lower id to be executed first
                    sort(readyvector_low.begin(), readyvector_low.end(), sort_period_time); //sort ready queue with deadline
        ...
        */
        sort(readyvector_low.begin(), readyvector_low.end(), sort_period_time); //sort low ready queue by waiting time
        //add process time
        ++process_time;
    }
...
void time_demand(vector<Task> waitvector, int schedule_num)
{
    //cout << "in time demad" << endl;
    if (waitvector.size() > 1)
        for (int i = 1; i < waitvector.size(); ++i)
        {
            for (float t = 0; t < waitvector[i].period; ++t)
            {
                int W = 0;
                int ceil_num = 0;
                for (int j = i - 1; j >= 0; --j)
                {
                    ceil_num += waitvector[j].execution_time * ceil(t / (float)waitvector[j].period);
                }
                W += waitvector[i].execution_time;
                if (W > waitvector[i].period)
                {
                    cout << "Test Case " << schedule_num << ": not schedulable" << endl;
                    return;
                }
            }
        }
    cout << "Test Case " << schedule_num << ": schedulable" << endl;
    return;
}
...
bool sort_period_time(Task task1, Task task2)
{
